...
// LÓGICA DE MODAL NATIVO (Sin Bootstrap)
function abrirModalProgramada(indice) {
  ...
  document.getElementById("modalProgramadaLabel").innerHTML = `<i class="bi bi-calendar-check text-primary"></i> Programación: ${d.numero_incidente} - ${d.numero}`;
  
  document.getElementById("modalProgramadaBody").innerHTML = `
    <div class="row g-3 mb-3">
      <div class="col-md-6">
        <label class="text-muted small d-block">CLIENTE</label>
        <p class="fw-bold mb-0" style="font-size:1.1rem;">${d.cliente}</p>
      ...
      <div class="col-md-6">
        ...
        <p class="mb-0"><span class="badge bg-warning text-dark px-3 py-2" style="font-size:1rem;">${d.fecha}</span></p>
  ...
  // Cambiamos el display a 'flex' para que se centre en la pantalla
  document.getElementById("modalProgramada").style.display = 'flex';
}

// Función global para cerrar el modal nativo
