...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnNext) {
    btnNext.addEventListener("click", function (e) {
      ...
      const totalPaginas =
        Math.ceil(incidenciasTotales.length / registrosPorPagina);
  ...
  document.querySelectorAll(".btn-filtro-rapido").forEach(button => {
    ...
    button.addEventListener("click", function () {
      ...
      document
        .querySelectorAll(".btn-filtro-rapido")
      ...
      if (filtro === "programadas") {
        ...
        const checkProg =
          document.getElementById("solo-programadas");

        if (checkProg) {
          checkProg.checked = true;
        }

      } else if (filtro !== "todos") {

        const selectEquipo =
          document.getElementById("tipo-equipo");

        const checkActivas =
          document.getElementById("solo-activas");

        if (selectEquipo) {
          selectEquipo.value = filtro;
        }

        if (checkActivas) {
          checkActivas.checked = true;
        }
      }
...
async function cargarIncidencias() {
  ...
  const url =
    `../backend/buscar_reportes.php?${new URLSearchParams(params).toString()}`;
  ...
  items.forEach((inc, indexArray) => {
    ...
    let enlaceHTML = esProgramado
      ...
      ? `
        <button
          type="button"
          class="btn btn-link p-0 fw-bold text-primary text-decoration-none"
          style="vertical-align: baseline;"
          onclick="abrirModalProgramada(${indiceGlobal})"
        >
      ...
      : `
        <a href="detalle.html?id=${inc.id}"
           class="text-decoration-none">

          ${inc.numero_incidente || "N/A"}
...
function abrirModalProgramada(indice) {
  ...
  const lista =
    d.detalles_completos
      ? d.detalles_completos.split("||")
      : [];
  ...
  const modalLabel =
    document.getElementById("modalProgramadaLabel");

  const modalBody =
    document.getElementById("modalProgramadaBody");

  if (modalLabel) {

    modalLabel.innerHTML =
      `Programación: ${d.numero_incidente} - ${d.numero}`;
  }
    ...
    modalBody.innerHTML = `
      <div class="row g-3 mb-3">
        ...
        <div class="col-md-6">
          ...
          <label class="text-muted small d-block mt-2">
            SUCURSAL
          </label>
      ...
      <div class="bg-light p-3 rounded">
        ...
        <h6 class="fw-bold mb-3">
          ...
          Equipos vinculados a la visita:
  ...
  const modalEl =
    document.getElementById("modalProgramada");
...
function actualizarControlesPaginacion() {
  ...
  if (btnNext) {
    ...
    btnNext.classList.toggle(
      "disabled",
      paginaActual >= Math.ceil(total / registrosPorPagina)
      || total === 0
    );
  }
...
async function cargarClientes() {
  ...
  try {
    ...
    const response =
      await fetch("../backend/obtener-clientes.php");
    ...
    console.warn("Error cargando clientes");
  }
}
