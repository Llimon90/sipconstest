...
// Agregar esta función al archivo soporte_backend.php
case 'buscar':
...
function buscarContenido($conn) {
    try {
        ...
        $stmtModelos = $conn->prepare("
            SELECT m.id, m.nombre, m.tipo_equipo, ma.nombre as marca_nombre, 'modelo' as tipo
            FROM modelos m 
            INNER JOIN marcas ma ON m.marca_id = ma.id 
        ...
        // Buscar en documentos (nombres de archivos)
        $stmtDocumentos = $conn->prepare("
            SELECT d.id, d.nombre_archivo, d.tipo_documento, d.ruta_archivo,
                   m.nombre as modelo_nombre, ma.nombre as marca_nombre, 'documento' as tipo
            FROM documentos d
            INNER JOIN modelos m ON d.modelo_id = m.id
            INNER JOIN marcas ma ON m.marca_id = ma.id
            WHERE d.nombre_archivo LIKE ?
            ORDER BY ma.nombre, m.nombre, d.tipo_documento
        ");
        $stmtDocumentos->execute([$terminoBusqueda]);
        ...
        $resultados = [
            ...
            'modelos' => $modelos,
            'documentos' => $documentos,
            'total' => count($marcas) + count($modelos) + count($documentos)
        ];
